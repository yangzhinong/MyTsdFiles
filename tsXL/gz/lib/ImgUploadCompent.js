...
var GZIMG;
(function (GZIMG) {
    function ImgUploadCompent($txts, paths, fcbReponse) {
    ...
    GZIMG.ImgUploadCompent = ImgUploadCompent;
    function InitMutiImgUpLoadButton($btns, typeId, opt) {
        for (var i = 0; i < $btns.length; i++) {
            var $btn = $btns.eq(i);
            var opt1 = $.extend({}, { Id: 0, paths: 'public/product' }, opt);
            opt1.Id = opt1.Id || Number($btn.attr("value")) || Number($btn.attr("data-id"));
            aBtnInit($btn, opt1);
        }
        function aBtnInit($btn, opt) {
    ...
    GZIMG.InitMutiImgUpLoadButton = InitMutiImgUpLoadButton;
    function InitMutiImgUpLoadButton2($btns, typeId, opt) {
        for (var i = 0; i < $btns.length; i++) {
            var $btn = $btns.eq(i);
            var opt1 = $.extend({}, { Id: 0, paths: 'public/product' }, opt);
            //debugger;
    ...
    GZIMG.InitMutiImgUpLoadButton2 = InitMutiImgUpLoadButton2;
})(GZIMG || (GZIMG = {}));
