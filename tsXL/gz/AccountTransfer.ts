$(document).ready(function () {
    ...
    var fLoadTourShopNoList = function () {
        ...
        $fg.show();
        let ops: IBoostrapSuggestOptions = {};
    ...
    function doAccountTransfer(cardno: string) {
        var dlg = new BootstrapDialog({
            ...
            message: function () {
                ...
                $msg.load("/GztCard/_AccountTransfer", { cardno: cardno },
                    function () {
                        //console.log('load:' + $msg.html());
                        $msg.find('#btn-swap-card').click(function () {
                            var fToggleDisable = function ($e: JQuery) {
                                if ($e.prop("disabled")) $e.removeAttr('disabled');
            ...
            buttons: [
                {
                    ...
                    label: "提交",
                    action: function () {
                        let $btn = <IBootstrapDialogButtonEx>this;
                        $btn.disable();
                        let $frm = dlg.$modalBody.find("form");
                        ...
                        $.ajax({
                            ...
                            type: "POST",
                            data: dInput,
                            success: function (data:IJsonMsg) {
                                layer.closeAll();
                                if (data.code) {
                                    layer.alert(data.msg, LayerIcon.SmillingFace, function () {
                            ...
                            error: function (data) {
                                ...
                                layer.alert(data.responseText, LayerIcon.CryingFace);
                            },
                            complete: function () {
